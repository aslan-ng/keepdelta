...
class Model:
    ...
    def update(self):
        ...
        if self.position[1] <= 0:
            self.position[1] = (
                -self.position[1] * self.energy_loss
            )  # Bounce back (estimation)
            self.velocity[1] = (
                -self.velocity[1] * self.energy_loss
            )  # Reverse and reduce vy
...
if __name__ == "__main__":
    ...
    model = Model(
        position=[0.0, 10.0], velocity=[1.0, 0.0], g=9.81, dt=0.01, energy_loss=0.8
    )
    steps = 1000
    result = {"initial_position": deepcopy(model.position), "deltas": []}
